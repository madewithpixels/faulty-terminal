...
const TUNING = {
  ...
  impactBurst: 2.00,
  // mwp
  mwpColorStrength: 0.78,
  mwpDensity: 0.18,
  mwpFlickerSpeed: 0.30,
...
function parseAttrOpts(ctn){
  ...
  // shorthand attributes win over data-ft-opts
  if(ctn.hasAttribute('data-ft-no-ripple')) overrides.ripple = false;
  if(ctn.dataset.ftRipple === 'false') overrides.ripple = false;
  if(ctn.dataset.ftRipple === 'true') overrides.ripple = true;
  if(ctn.dataset.ftTint) overrides.tint = ctn.dataset.ftTint;
  if(ctn.dataset.ftBg) overrides.bgColor = ctn.dataset.ftBg;
  if(ctn.dataset.ftOrigin) overrides.rippleOriginSelector = ctn.dataset.ftOrigin;
  return overrides;
}
function siteConfig(){
